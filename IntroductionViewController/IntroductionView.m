...
@synthesize pageControllOffset = _pageControllOffset;
...
- (CGRect)frameOfPageControl{
    ...
    return CGRectOffset(orgFrame, self.pageControllOffset.x, self.pageControllOffset.y);
}

- (CGPoint)pageControllOffset{
    if (CGPointEqualToPoint(_pageControllOffset, CGPointZero)) {
        return CGPointMake(0, -30);
    }
    return _pageControllOffset;
}
...
- (UIView *)pageViewWithTitle:(NSString *)title{
    ...
    rect = CGRectMake(0, size.height + self.pageControllOffset.y - height, size.width, height);
...
- (NSArray *)scrollViewPages{
    ...
    NSMutableArray *tmpArray = [NSMutableArray new];
    if (self.coverTitles) {
        [self.coverTitles enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
